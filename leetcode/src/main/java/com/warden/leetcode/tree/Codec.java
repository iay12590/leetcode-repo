...
public class Codec {
	...
	public TreeNode deserialize(String data) {
		...
		String[] nodeArr = trim.split(",");
		if (nodeArr.length <= 0) {
			return null;
		}
		TreeNode root  = getNode(nodeArr, 0);
		Queue<TreeNode> queue = new LinkedList<>();
		queue.add(root);
		int index = 1;
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < size; i++) {
				TreeNode node = queue.poll();
				TreeNode left = getNode(nodeArr, index++);
				TreeNode right = getNode(nodeArr, index++);
				node.left = left;
				node.right = right;
				if (left != null) {
					queue.add(left);
